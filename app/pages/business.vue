...
useSeoMeta({
  title: 'Navitag Business — Fleet Tracking & Enterprise Plans',
  description: 'Volume pricing, on-site installation, 24/7 support, lifetime warranty, and daily fleet reports. Built for operators of 1 to 1,000+ devices.',
  ogUrl: 'https://navitag.com/business',
})
useHead({ link: [{ rel: 'canonical', href: 'https://navitag.com/business' }] })
...
const fleetCopy: FleetCopy[] = [
  {
    label: 'Starter Fleet',
    tagline: 'Small teams getting started with connected operations.',
  },
  {
    label: 'Growth Fleet',
    tagline: 'Expanding operations with a growing vehicle roster.',
  },
  {
    label: 'Enterprise Fleet',
    tagline: 'Large operators. Volume discounts and custom terms.',
  },
]

const { tiers: fleetData } = await useFleetPricing('US')
...
const services: Service[] = [
  {
    ...
    body: 'Our field technicians handle mounting, wiring, and activation for every device in your fleet.',
    tag: 'Within coverage area',
  },
  {
  ...
  },
  {
    ...
    body: 'If a device fails under warranty, we come to you and swap it — no shipping delays.',
    tag: 'Within coverage area',
  },
  {
    icon: 'fa-headset',
    title: '24/7 dedicated support',
    body: 'Priority phone, email, and chat support with real humans — day, night, weekends, holidays.',
  },
  {
    icon: 'fa-shield-alt',
  ...
  },
  {
    icon: 'fa-chart-line',
    title: 'Daily reports',
    body: 'Per-device activity reports and full fleet overview emailed every morning, plus on-demand in dashboard.',
  },
]
...
const includedFeatures: IncludedFeature[] = [
  ...
  { icon: 'fa-users', label: 'Role-based shared access' },
  { icon: 'fa-webhook', label: 'Webhooks · email · app alerts' },
  { icon: 'fa-globe', label: '100+ country M2M coverage' },
  { icon: 'fa-sim-card', label: '2G / 3G / 4G / 5G multi-gen' },
...
<template>
      ...
      <div class="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
        <div class="lg:col-span-7">
          ...
          <h1 class="mt-5 text-4xl sm:text-5xl lg:text-[64px] font-semibold tracking-tight leading-[1.02] text-gray-950">
            Fleet tracking that <span class="text-navitag-blue">pays back</span> every day.
          </h1>
          <p class="mt-6 text-base sm:text-lg text-gray-600 max-w-xl leading-relaxed">
            Volume pricing, on-site installation, 24/7 support, and daily reports — built for operators running anywhere from a handful of vehicles to thousands.
          </p>
          <div class="mt-8 flex flex-col sm:flex-row gap-3">
            <NuxtLink
              to="/contact?subject=Navitag%20Business%20Inquiry"
              ...
              data-pixel-content-name="business_hero"
          ...
          <dl class="mt-12 grid grid-cols-3 gap-4 sm:gap-8 max-w-2xl">
            ...
            <div>
              <dt class="text-[11px] uppercase tracking-[0.18em] text-gray-400 font-semibold">Reports</dt>
              <dd class="mt-1 text-2xl sm:text-3xl font-semibold tracking-tight text-gray-950">Daily</dd>
            </div>
          </dl>
        </div>
            ...
            <div class="absolute inset-6 grid grid-rows-[auto_1fr_auto] gap-4">
              ...
              <div class="rounded-2xl bg-white border border-gray-200 p-4 shadow-sm">
                ...
                <div class="mt-2 flex items-baseline gap-2">
                  <span class="text-3xl font-semibold tracking-tight text-gray-950">142</span>
                  <span class="text-[12px] text-green-600 font-medium">↑ 8 from yesterday</span>
                </div>
              </div>

              <!-- Middle: sparkline-ish bars -->
              <div class="relative rounded-2xl bg-gradient-to-br from-navitag-blue/95 to-[#0A60C8] p-5 overflow-hidden">
                ...
                <div class="mt-2 text-2xl font-semibold tracking-tight text-white">8,420 km</div>
  ...
  <!-- Fleet pricing -->
  <section id="fleet-pricing" class="relative bg-white py-20 sm:py-28 border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-6 sm:px-8">
      <div class="max-w-3xl">
        ...
        <p class="mt-4 text-[15px] text-gray-600 leading-relaxed max-w-2xl">
          The more devices you run, the less you pay per device. Every tier includes 24/7 support, lifetime warranty, and daily reports.
        </p>
      </div>
      ...
      <div class="mt-8 grid md:grid-cols-3 gap-5 lg:gap-6">
        ...
        >
          ...
          <!-- CTA -->
          <NuxtLink
            :to="`/contact?subject=${encodeURIComponent('Navitag Business - ' + tier.label + ' (' + tier.range + ' units)')}`"
            ...
            data-pixel-audience="b2b"
            :data-pixel-content-name="`fleet_${tier.label.toLowerCase().replace(/\s+/g, '_')}`"
      ...
      <div class="mt-12 rounded-3xl bg-navitag-bg border border-gray-200/70 p-6 sm:p-8">
        <div class="flex items-baseline justify-between flex-wrap gap-2">
          <div class="text-[11px] uppercase tracking-[0.2em] font-semibold text-gray-500">
            Included in every Business plan
          </div>
          <NuxtLink to="/data-plans" class="text-[12px] text-navitag-blue font-semibold hover:underline">
            Compare with Basic &amp; Pro →
          </NuxtLink>
        </div>
        <div class="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-4">
          <div
            v-for="f in includedFeatures"
            :key="f.label"
            class="flex items-center gap-3 text-[13.5px] text-gray-700"
          >
            <span class="shrink-0 w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-navitag-blue">
  ...
  <!-- Service benefits -->
  <section class="relative bg-navitag-bg py-20 sm:py-28">
    <div class="max-w-7xl mx-auto px-6 sm:px-8">
      <div class="max-w-3xl">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-gray-200 text-gray-700 text-[11px] font-semibold tracking-[0.15em] uppercase">
          White-glove service
      ...
      <div class="mt-8 flex items-start gap-4 max-w-3xl">
        <div class="shrink-0 w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-500">
          <i class="fas fa-info text-[12px]"></i>
        </div>
        <p class="text-[13.5px] text-gray-600 leading-relaxed">
          On-site installation, inspection, and replacement are available within our serviced metro areas. Outside coverage zones, we cover round-trip shipping for warranty replacements.
        </p>
      </div>
    </div>
  </section>
        ...
        <div class="relative grid sm:grid-cols-[1fr_auto] gap-6 items-end">
          <div>
            ...
            <h3 class="mt-3 text-3xl sm:text-4xl font-semibold tracking-tight leading-tight">
              Let's scope your fleet. <br class="hidden sm:block">We'll send a tailored quote within one business day.
            </h3>
          </div>
          <NuxtLink
            to="/contact?subject=Navitag%20Business%20Inquiry"
            ...
            data-pixel-event="Contact"
            data-pixel-audience="b2b"
            data-pixel-content-name="business_bottom_cta"
            data-pixel-content-category="b2b_intent"
            data-pixel-lead-type="business_inquiry"
          >
            Contact us
