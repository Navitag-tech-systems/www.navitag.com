...
- **Data Plan Top-Up Flow**: Complete end-to-end flow built — device lookup via Unified API (`/top-up/:imei`), tiered plan selection (Basic/Pro) with duration variants and pricing, cart creation with IMEI metadata and digital delivery shipping, checkout page (`/plan-checkout/:cart_id`) with PayPal inline card fields (Alphabit plugin, sandbox mode, 3DS/OTP support), and top-up confirmation page (`/renew-complete/:order_id`) with device info, payment summary, and activation status.
